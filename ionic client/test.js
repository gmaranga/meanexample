  ...
  describe('tab-search.html', function() {
    ...
    it('has the ion-view, ion-content, and search-bar directives', function(done) {
      scope.$on('SearchBarController', function() {
        ...
        assert.equal(element.find('ion-view').find('ion-content').length, 1);
        var matchs = element[0].querySelectorAll('.search-bar-wrapper');
        assert.equal(matchs.length, 1);
    ...
    it('has a functioning search-bar directive', function(done) {
      ...
      scope.$on('SearchBarController', function(event) {
        scope.searchText = "test";
        scope.$apply();
        var value = element.find('input').val();
        event.targetScope.update();
        assert.equal(value,'test');

        httpBackend.flush();

       assert.equal(event.targetScope.results.length, 2);
       assert.equal(event.targetScope.results[0].name, 'test1');
       assert.equal(event.targetScope.results[1].name, 'test2');
